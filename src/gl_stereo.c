...
glconfig = gdis_gl_config_new(FALSE);
#else

/* non SGI - request a proper stereo canvas */
glconfig = gdis_gl_config_new(TRUE);
...
stereo_glarea = gdis_gl_widget_new(glconfig);
...
/* set cursor */
glwindow = gdis_gl_get_window(stereo_glarea);
if (glwindow)
  gdk_window_set_cursor(glwindow, cursor);
}
#endif
