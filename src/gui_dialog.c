...
combo = gui_combo_new(elist, FALSE);
...
if (fdata)
  gui_combo_set_text(combo, fdata->label);
...
if (GTK_IS_WIDGET(cs))
  gdis_colour_chooser_get_current_color(cs, &colour);
if (GTK_IS_WIDGET(w))
  gdis_widget_set_background_colour(w, &colour);
}
...
csd = gdis_colour_dialog_new("Edit colour");

/* set the initial colour */
gdis_colour_chooser_set_rgb(gdis_colour_dialog_get_chooser(csd), colour);

/* value colour update */
gdis_colour_chooser_connect_changed(gdis_colour_dialog_get_chooser(csd),
                                    GTK_SIGNAL_FUNC(set_colour_value),
                                    (gpointer) colour);
/* widget colour update */
if (w)
  gdis_colour_chooser_connect_changed(gdis_colour_dialog_get_chooser(csd),
                                      GTK_SIGNAL_FUNC(set_colour_widget),
                                      (gpointer) w);

g_signal_connect(GTK_OBJECT(gdis_colour_dialog_get_ok_button(csd)), "clicked",
                 GTK_SIGNAL_FUNC(destroy_widget), (gpointer) csd);

gtk_widget_hide(gdis_colour_dialog_get_cancel_button(csd));
