...
g_timeout_add(100, siesta_phonon_timer, dialog);
}
...
//  Single Point Options
    ...
    combo = gui_combo_new(list, FALSE);
    switch (model->siesta.md_type_of_run)
      {
      case NOSE_MDRUN:
        gui_combo_set_text(combo, "Nose");
        break;
      case PARRINELLOPAHMAN_MDRUN:
        gui_combo_set_text(combo, "Parrinello-Rahman");
        break;
      case NOSEPARRINELLOPAHMAN_MDRUN:
        gui_combo_set_text(combo, "Nose-Parrinello-Rahman");
        break;
      case ANNEAL_MDRUN:
        gui_combo_set_text(combo, "Anneal");
        break;
      case PHONON_MDRUN:
        gui_combo_set_text(combo, "Phonon");
        break;
      case VERLET_MDRUN:
      default:
        gui_combo_set_text(combo, "Verlet");
        break;
      }
...
void set_md_run_type(GtkWidget *w, gpointer *data)
{
    ...
    text = gui_combo_text(GTK_WIDGET(data));
    if (!text)
        return;

    if (g_ascii_strncasecmp("Verlet", text, 6) == 0)
        model->siesta.md_type_of_run = VERLET_MDRUN;
    else if (g_ascii_strncasecmp("Nose", text, 4) == 0)
        model->siesta.md_type_of_run = NOSE_MDRUN;
    else if (g_ascii_strncasecmp("Parrinello-Rahman", text, 17) == 0)
        model->siesta.md_type_of_run = PARRINELLOPAHMAN_MDRUN;
    else if (g_ascii_strncasecmp("Nose-Parrinello-Rahman", text, 22) == 0)
        model->siesta.md_type_of_run = NOSEPARRINELLOPAHMAN_MDRUN;
    else if (g_ascii_strncasecmp("Anneal", text, 6) == 0)
        model->siesta.md_type_of_run = ANNEAL_MDRUN;
    else if (g_ascii_strncasecmp("Phonon", text, 6) == 0)
