...
fn = gdis_font_dialog_get_font_name(fsd);
...
fsd = gdis_font_dialog_new("Font selection");

/* setup events for the file selection widget */
g_signal_connect(GTK_OBJECT(gdis_font_dialog_get_ok_button(fsd)),
                 "clicked", (GtkSignalFunc) gl_font_selected, fsd);
g_signal_connect(GTK_OBJECT(gdis_font_dialog_get_cancel_button(fsd)),
...
switch(id)
  ...
  case ANIM_NAME:
    ...
    break;
  case LIGHT_TYPE:
    entry = gui_combo_text(GTK_WIDGET(obj));
...
if (camera->perspective)
  gui_pulldown_set_text(camera_proj_entry, "perspective");
else
  gui_pulldown_set_text(camera_proj_entry, "orthographic");
...
if (camera->mode == LOCKED)
  gui_pulldown_set_text(camera_mode_entry, "origin");
else
  gui_pulldown_set_text(camera_mode_entry, "variable");
...
camera_axis_entry = gui_combo_entry(combo);
...
g_signal_connect(GTK_OBJECT(gui_combo_entry(combo)), "changed",
