  ...
  if (g_ascii_strncasecmp(line, "  Elastic Compliance Matrix:", 28) == 0)
    ...
    for (i=0 ; i<6 ; i++)
      ...
      if (tokens > 6)
        ...
        {
        gui_text_show(ERROR, "Error parsing elastic compliance constants.\n");
        g_strfreev(buff);
        scan_free(scan);
        unlink(full_inp);
        unlink(full_out);
...
if (fabs(r44) < FRACTION_TOLERANCE || fabs(r55) < FRACTION_TOLERANCE)
  {
  gui_text_show(ERROR, "Bad elastic compliance matrix.\n");
  return;
  }
...
if (defect_compliance_get(s, plane, source))
  {
  gui_text_show(ERROR, "Failed to acquire the elastic compliance matrix.\n");
  plane_free(plane);
  return;
  }
else
...
defect_screw_dislocate(s, b, model);
  ...
  {
  gui_text_show(ERROR, "Only screw dislocations are implemented currently.\n");
  model_free(model);
  plane_free(plane);
  return;
  }
