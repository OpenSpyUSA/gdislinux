...
/***************************************/
gboolean gui_widget_handler(void)
{
...
sysenv.depth = gdis_gdk_visual_get_depth(sysenv.visual);
...
sysenv.gtk_fontsize = gdis_gtk_widget_font_size(window);
...
pixbuf = gdk_pixbuf_new_from_xpm_data((const char **) logo_left_xpm);
gdis_wid = gtk_image_new_from_pixbuf(pixbuf);
g_object_unref(pixbuf);
gtk_box_pack_start(GTK_BOX(hbox), gdis_wid, FALSE, FALSE, 0);


pixbuf = gdk_pixbuf_new_from_xpm_data((const char **) logo_right_81_xpm);
gdis_wid = gtk_image_new_from_pixbuf(pixbuf);
g_object_unref(pixbuf);
