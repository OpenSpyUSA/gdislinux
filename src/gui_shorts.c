...
csd = gdis_colour_dialog_new(title);

/* set up callbacks */
g_signal_connect_swapped(G_OBJECT(gdis_colour_dialog_get_ok_button(csd)),
                        "clicked", 
                         GTK_SIGNAL_FUNC(callback),
                        (gpointer) gdis_colour_dialog_get_chooser(csd));

g_signal_connect_swapped(G_OBJECT(gdis_colour_dialog_get_cancel_button(csd)),
                        "clicked",
...
gdis_pixb = gdk_pixbuf_new_from_xpm_data ((const char **)pause_xpm);
  image = gtk_image_new_from_pixbuf(gdis_pixb);
  g_object_unref(gdis_pixb);
  stock = FALSE;
  }
...
static GtkWidget *gdis_combo_entry_get(GtkWidget *combo)
{
GtkWidget *child;
...
GtkWidget *gui_combo_new(GList *list, gint edit)
{
GtkWidget *combo;
GtkWidget *entry;
GList *item;

combo = gdis_gtk_combo_box_text_new(TRUE);
...
if (GTK_IS_EDITABLE(entry))
  gtk_editable_set_editable(GTK_EDITABLE(entry), edit);

return(combo);
}

GtkWidget *gui_combo_entry(GtkWidget *combo)
{
return(gdis_combo_entry_get(combo));
}

const gchar *gui_combo_text(GtkWidget *combo)
{
return(gui_pulldown_text(combo));
}

void gui_combo_set_text(GtkWidget *combo, const gchar *text)
{
gui_pulldown_set_text(combo, text);
}
...
combo = gdis_gtk_combo_box_text_new(edit);
for (item=list ; item ; item=g_list_next(item))
  gtk_combo_box_append_text(GTK_COMBO_BOX(combo), item->data);
...
entry = gdis_combo_entry_get(GTK_WIDGET(w));
if (entry)
...
g_object_set_data_full(G_OBJECT(w), "gdis-pulldown-text", text, g_free);
return(g_object_get_data(G_OBJECT(w), "gdis-pulldown-text"));
}
...
while (valid)
  ...
  if (item && g_ascii_strcasecmp(item, text) == 0)
    {
