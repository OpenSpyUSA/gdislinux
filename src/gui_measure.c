...
label[0] = gui_combo_text(match1);
label[1] = gui_combo_text(match2);
...
search = gui_combo_new(search_list, FALSE);
gtk_box_pack_start(GTK_BOX(hbox),search,FALSE,FALSE,0); 
gui_combo_set_text(search, "Bonds");
g_signal_connect(GTK_OBJECT(search), "changed", 
...
match1 = gui_combo_new(match_list, TRUE);
gui_combo_set_text(match1, "Any");
gtk_table_attach_defaults(GTK_TABLE(table),match1,0,1,1,2);

match2 = gui_combo_new(match_list, TRUE);
gui_combo_set_text(match2, "Any");
gtk_table_attach_defaults(GTK_TABLE(table),match2,1,2,1,2);

match3 = gui_combo_new(match_list, TRUE);
gui_combo_set_text(match3, "Any");
