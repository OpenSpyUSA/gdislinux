...
/* missing default element info is fatal */
  fprintf(stderr, "Error: required element file not found: %s\n", sysenv.elem_file);
  exit(1);
  }
...
#ifdef WITH_GUI
static gboolean update_task_info_cb(gpointer data)
{
(void) data;
...
static gboolean gui_widget_handler_cb(gpointer data)
{
(void) data;

return(gui_widget_handler());
}
#endif
...
/* gui_init() owns GTK and OpenGL backend initialization. */
...
/* TODO - put this in gui_widget_handler? */
  g_timeout_add(1000, update_task_info_cb, NULL);

/* screen redraw timer */
  g_timeout_add(25, gui_canvas_handler, NULL);

/* CURRENT - gui updates done through a timer */
/* this was done so that threads can safely schedule a gui update */
  g_timeout_add(200, gui_widget_handler_cb, NULL);
